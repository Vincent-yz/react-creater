...
const Runtime =      require(path.join(jestModulePath, 'jest-runtime'));
const SearchSource = require(path.join(jestPath, './SearchSource'));
const TestRunner =   require(path.join(jestPath, './TestRunner'));

var _require =  require(path.join(jestModulePath, 'jest-util'));
const Console = _require.Console,
	clearLine =   _require.clearLine;
var _require2 = require(path.join(jestModulePath, 'jest-util'));
const formatTestResults = _require2.formatTestResults;
var _require3 =           require(path.join(jestPath, './cli'));
// const run =               _require3.run;
const ansiEscapes =       require(path.join(jestModulePath, 'ansi-escapes'));
const chalk = require('chalk');
const os =    require('os');
const preRunMessage =     require(path.join(jestPath, './preRunMessage'));
// const readConfig =        require(path.join(jestModulePath, 'jest-config')).readConfig;
const readConfig =        require('./jest-config').readConfig;
const sane =              require(path.join(jestModulePath, 'sane'));
const which =             require(path.join(jestModulePath, 'which'));
const TestWatcher =       require(path.join(jestPath, './TestWatcher'));

const CLEAR = process.platform === 'win32' ? '\x1Bc' : '\x1B[2J\x1B[3J\x1B[H';
const VERSION = require(path.join(jestPath, '../package.json')).version;
...
const args    = require(path.join(jestPath, 'cli', './args'));
const getJest = require('./getJest');
const getPackageRoot = require(path.join(jestModulePath, 'jest-util')).getPackageRoot;
const warnAboutUnrecognizedOptions = require(path.join(jestModulePath, 'jest-util')).warnAboutUnrecognizedOptions;
